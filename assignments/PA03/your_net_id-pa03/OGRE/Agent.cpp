...
Agent::Agent(Ogre::SceneManager* SceneManager, std::string name, std::string filename, float height, float scale, Grid * grid)
{
	...
	this->addMoreWayPoints(10); //add 10 waypoints to start
}
...
void Agent::setPosition(float x, float y, float z)
{
	...
	m_cur_node = m_grid->getNode(mBodyNode->getPosition());
	if (m_cur_node != NULL) {
		m_cur_node_state = m_cur_node->isClear();
		m_cur_node->setOccupied();
	}
...
Agent::updateLocomote(Ogre::Real deltaTime)
{
	...
	else
    {
		...
		if (mDistance <= 0.0f)
        {
			...
			this->setPosition(mDestination.x, mDestination.y-this->height, mDestination.z);
            mDirection = Ogre::Vector3::ZERO;
...
void Agent::addMoreWayPoints(int number)
{
	// create random points to walk to 
	float x, z;
	for (int i = 0; i < number; i++)
	{
		x = rand() % m_grid->getWidth() - m_grid->getWidth() / 2;
		z = rand() % m_grid->getHeight() - m_grid->getHeight() / 2;
		mWalkList.push_back(Ogre::Vector3(x, this->height, z));
	}
}
